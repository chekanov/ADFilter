...
#
# The goal of this tool is to convert DAOD_PHYSLIGHT to Ntuple which can be uploaded
# to the ADFilter tool: https://mc.hep.anl.gov/asc/adfilter
# 
# How to run over DAOD_PHYS:
#   lsetup "asetup Athena,main,latest"
#   python ./DAOD_PHYSLIGHT2Ntuple.py  --inputlist DAOD_PHYSLIGHT.37222100._000003.pool.root.1  --outputlist output.root --cmsEnergy 13000
#   
...
def main(filenameinput, filenameoutput, cmsEnergy, cross ):
    ...
    for idx in range(0, evtMax):
        ...
        jets = evt.retrieve('xAOD::JetContainer', 'AnalysisJets')
        ...
        for jet in jets:
            ...
            GN2v01_pb= None if not btagInfo.isValid() else btagInfo.auxdataConst["float"]("GN2v00_pb")
            GN2v01_pc= None if not btagInfo.isValid() else btagInfo.auxdataConst["float"]("GN2v00_pc")
            GN2v01_pu= None if not btagInfo.isValid() else btagInfo.auxdataConst["float"]("GN2v00_pu")
            # GN2v01_ptau= None if not btagInfo.isValid() else btagInfo.auxdataConst["float"]("GN2v00_ptau")
            GN2v01_ptau=0
        ...
        electrons = evt.retrieve('xAOD::ElectronContainer', 'AnalysisElectrons')
        for el in electrons:
        ...
        muons = evt.retrieve('xAOD::MuonContainer', 'AnalysisMuons')
        for mu in muons:
        ...
        photons = evt.retrieve('xAOD::PhotonContainer', 'AnalysisPhotons')
        for ph in photons:
            topoetcone40=ph.auxdataConst["float"]("topoetcone40")
            # tight=ord(ph.auxdataConst["char"]("Tight"))
            tight=ord(ph.auxdataConst["char"]("DFCommonPhotonsIsEMTight"))
        ...
        met = evt.retrieve('xAOD::MissingETContainer', 'MET_Core_AnalysisMET')
        for m in met:
              x=MeVtoGeV*m.mpx()
              y=MeVtoGeV*m.mpy()
              phi = math.atan2(y, x)
