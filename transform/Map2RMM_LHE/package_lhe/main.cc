...
int main(int argc, char* argv[]) {
  ...
  if (argc != 4) {
   ...
   cout << "HepSim:  Input LHE  =" << argv[2] << endl;
   cout << "HepSim:  ProMC Output =" << argv[3] << endl;
   
  // see lhe2promc for detail. 
  // calculate numbers of events 
  string infile("-");
  infile = argv[2];
  ...
  while ( reader.readEvent() ) {
      iEvent++;
      int nn=reader.hepeup.NUP;
  };
  cout << "Total events found in LHE :" << iEvent << endl;
...
/////////// read config files ////////////////////
    ...
    Ntot= iEvent;  // atoi(readnum[1].c_str()); 
 ...
 // no caching
 ProMCBook*  epbook = new ProMCBook(argv[3],"w");
 ...
 // If failure because reached end of file then exit event loop.
    if (pythia.info.atEndOfFile()) {
          cout << "END of LHE file.. Exit. " << endl;
          break;
    }
